...
nothing under `safety/` is imported by the package.

## Subdirectories

| Path | Class | Role |
|---|---|---|
| [`tests/`](tests/) | active tests | The full pytest suite (1200+ tests): schemas, classify, policy, projectors, migrations, CLI surfaces, atomic-transaction semantics, skill-boundary contracts, eval runner, scenario pack. Run with `uv run pytest safety/tests -q`. |
...
`safety/` also contains two committed symlinks:

| Symlink | Target | Purpose |
|---|---|---|
| `safety/artifacts` | `../artifacts/` | Lets tests and local replays reach into the gitignored top-level `artifacts/` directory using a stable relative path. |
| `safety/data` | `../data/` | Same idea for the gitignored top-level `data/` runtime data root. |
